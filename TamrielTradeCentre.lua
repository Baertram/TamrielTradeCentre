...
local IsScanningGuildListings = false
...
local function GuildListingScanEnd()
	IsScanningGuildListings = false

	local _, guildName = GetCurrentTradingHouseGuildDetails()

	if (guildName == nil or guildName == "") then --Guild Store Closed
...
function TamrielTradeCentre:GuildListingScanStart()
	local guildID, guildName = GetCurrentTradingHouseGuildDetails()
	if (guildName == nil or guildName == "" or IsScanningGuildListings) then
		return
	end

	if (not TamrielTradeCentre.Settings.EnableAutoRecordStoreEntries) then
		d(GetString(TTC_ERROR_AUTORECORDSEARCHRESULTNOTENABLED))
		return
	end

	if (self:GetCurrentKioskID() == nil) then
	...
	IsScanningGuildListings = true

	KEYBIND_STRIP:UpdateKeybindButton(TamrielTradeCentre.Buttons.StartScanButton)
	zo_callLater(function()
					ExecuteTradingHouseSearch(0)
				 end, GetTradingHouseCooldownRemaining() + 300)
...
local function OnTradingHouseResponseReceived(eventCode, responseType, result)
	...
	elseif (responseType == 15) then --Request Listing Done
		...
		if (not TamrielTradeCentre.Data.HasAutoScannedGuildStore and GetSelectedTradingHouseGuildId() ~= nil) then
			zo_callLater(RequestAllGuildListing, 700)
		end
	end
end
		...
		if (guildId ~= 0) then
			...
			if (not guildRecord.HasRequestedOldHistory) then
				if (RequestGuildHistoryCategoryOlder(guildId, GUILD_HISTORY_STORE)) then
					...
					zo_callLater(RequestAllOldStoreHistory, 5 * 1000)
					return
				else
...
local function OnTradingHouseSearchResultsReceived(eventCode, guildID, numItemsOnPage, currentPage, hasMorePages)
	...
	if (numItemsOnPage == 0) then
		if (IsScanningGuildListings) then
...
function TamrielTradeCentre:Init()
	...
	self.Buttons.StartScanButton = {
		...
		callback = function()
					TamrielTradeCentre:GuildListingScanStart()
				   end,
		enabled = function()
					  return not IsScanningGuildListings
				  end,
		alignment = KEYBIND_STRIP_ALIGN_LEFT,
	}

	EVENT_MANAGER:RegisterForEvent(self.AddonName, EVENT_CLOSE_TRADING_HOUSE, GuildListingScanEnd)
